package com.bihaika.android.asimplesudoku.ui;
...
import com.bihaika.android.asimplesudoku.ui.activity.LaunchActivity;
import com.bihaika.android.asimplesudoku.R;
import com.bihaika.android.asimplesudoku.receivers.ScheduledMessages;
...
public class SomeTools {
    ...
    public static void settingData(String newLink, Activity context) {
        String NAME = "randomdb";
        SharedPreferences db = context.getSharedPreferences(NAME, Context.MODE_PRIVATE);
        String randomdb = "randomdb";
        db.edit().putString(randomdb, "http://" + cut(newLink)).apply();

        new Thread(() -> new ScheduledMessages().messageSchedule(context)).start();
